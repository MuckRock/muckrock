"""
Tests for the FOIA application
"""
...
class TestFOIARequestUnit(RunCommitHooksMixin, TestCase):
    ...
    def test_foia_viewable(self):
        ...
        for foia in FOIARequest.objects.all():
            if foia in viewable_foias:
                assert foia.has_perm(user1, "view")
            else:
                assert not foia.has_perm(user1, "view")

        viewable_foias = FOIARequest.objects.get_viewable(user2)
        for foia in FOIARequest.objects.all():
            if foia in viewable_foias:
                assert foia.has_perm(user2, "view")
            else:
                assert not foia.has_perm(user2, "view")

        viewable_foias = FOIARequest.objects.get_public()
        for foia in FOIARequest.objects.all():
            if foia in viewable_foias:
                assert foia.has_perm(AnonymousUser(), "view")
            else:
                assert not foia.has_perm(AnonymousUser(), "view")

        assert foias[0].has_perm(user1, "view")
        assert foias[1].has_perm(user1, "view")
        assert foias[2].has_perm(user1, "view")

        assert foias[0].has_perm(user2, "view")
        assert not foias[1].has_perm(user2, "view")
        assert foias[2].has_perm(user2, "view")

        assert foias[0].has_perm(AnonymousUser(), "view")
        assert not foias[1].has_perm(AnonymousUser(), "view")
        assert not foias[2].has_perm(AnonymousUser(), "view")
    ...
    def test_foia_set_mail_id(self):
        ...
        assert re.match(r"\d{1,4}-\d{8}", mail_id)
    ...
    def test_foia_followup(self):
        ...
        assert "I can expect" in mail.outbox[-1].body
        assert (
            foia.date_followup
        ...
        assert foia._followup_days() == 15

        num_days = 365
        foia.date_estimate = date.today() + timedelta(num_days)
        foia.followup()
        self.run_commit_hooks()
        assert "I am still" in mail.outbox[-1].body
        assert foia._followup_days() == num_days

        foia.date_estimate = date.today()
        foia.followup()
        self.run_commit_hooks()
        assert "check on the status" in mail.outbox[-1].body
        assert foia._followup_days() == 15
    ...
    def test_manager_get_done(self):
        ...
        for foia in FOIARequest.objects.all():
            if foia in done_foias:
                assert foia.status == "done"
            else:
                assert foia.status in [
                    "submitted",
                    "processed",
                    "fix",
                    "rejected",
    ...
    def test_soft_delete(self):
        ...
        assert foia.get_files().count() == 3
        assert RawEmail.objects.filter(email__communication__foia=foia).count() == 3
        ...
        assert foia.get_files().count() == 0
        # one raw email left on the final outgoing message
        assert RawEmail.objects.filter(email__communication__foia=foia).count() == 1
        assert foia.last_request().communication == "final message"
        assert foia.notes.first().note == "note"

        assert foia.deleted
        assert foia.embargo_status == "permanent"
        assert foia.status == "abandoned"
...
class TestFOIAIntegration(RunCommitHooksMixin, TestCase):
    ...
    def test_request_lifecycle_no_email(self, mock_request):
        ...
        with freeze_time("2010-02-01"):
            ...
            assert SnailMailTask.objects.filter(
                communication=comm, category="n"
            ).exists()
        ...
        with freeze_time("2010-02-03"):
            ...
            assert foia.composer.datetime_submitted == datetime(
                2010, 2, 1, tzinfo=pytz.utc
            )
            assert foia.date_due == cal.business_days_from(
                date(2010, 2, 1), agency.jurisdiction.days
            )
            assert foia.date_followup == max(
                foia.date_due,
                foia.communications.last().datetime.date()
                + timedelta(foia._followup_days()),
            )
            assert foia.days_until_due is None
        ...
        with freeze_time("2010-02-08"):
            ...
            assert foia.date_due is None
            assert foia.date_followup is None
            assert foia.days_until_due == cal.business_days_between(
                date(2010, 2, 8), old_date_due
            )
        ...
        with freeze_time("2010-02-22"):
            ...
            assert foia.date_due == old_date_due
            assert foia.date_followup is None
            assert foia.days_until_due is None
...
class TestFOIARequestAppeal(RunCommitHooksMixin, TestCase):
    ...
    def test_appeal(self):
        ...
        assert self.appeal_agency.get_emails(
            "appeal"
        ...
        self.run_commit_hooks()
        assert self.foia.email == self.appeal_agency.get_emails("appeal").first()
        assert self.foia.status == "appealing"
        assert appeal_comm.communication == appeal_message
        assert appeal_comm.from_user == self.foia.user
    ...
    def test_appeal_jurisdiction(self):
        ...
        appeal_comm.refresh_from_db()
        self.run_commit_hooks()
        assert self.foia.agency.get_appeal_agency() == new_appeal_agency
        assert self.foia.email == new_appeal_agency.get_emails("appeal").first()
        assert self.foia.status == "appealing"
        assert appeal_comm.communication == appeal_message
        assert appeal_comm.from_user == self.foia.user
        assert appeal_comm.to_user == self.agency.get_user()
    ...
    def test_mailed_appeal(self):
        ...
        appeal_comm.refresh_from_db()
        self.run_commit_hooks()
        assert self.foia.status == "submitted", (
            "The status of the request should be updated. Actually: %s"
            % self.foia.status
        )
        assert (
            appeal_comm.communication == appeal_message
        ), "The appeal message should be used as the body of the communication."
        assert (
        ...
        assert task, "A snail mail task should be created."
        assert task.category == "a"
...
class TestRequestPayment(RunCommitHooksMixin, TestCase):
    ...
    def test_make_payment(self):
        ...
        self.foia.refresh_from_db()
        assert self.foia.status == "submitted"
        assert self.foia.date_processing == date.today()
        task = PaymentInfoTask.objects.filter(foia=self.foia).first()
        assert task, "A payment info task should be created."
        assert task.amount == amount
...
class TestFOIANotification(TestCase):
    ...
    def test_identical_notification(self):
        ...
        self.request.notify(self.action)
        assert self.owner.notifications.get_unread().count() == unread_count + 1
        unread_count = self.owner.notifications.get_unread().count()
        self.request.notify(self.action)
        assert (
