...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Change all processed requests with no repsonse to waiting for acknowledgement"""
        for foia in orm.FOIARequest.objects.filter(status='processed'):
            if not foia.communications.filter(response=True).exists():
                foia.status = 'ack'
                foia.save()

    def backwards(self, orm):
        """Change all requests waiting for acknowledgement to processed"""
        for foia in orm.FOIARequest.objects.filter(status='ack'):
    ...
    models = {
        ...
        'foia.foiafile': {
            ...
            'foia': ('django.db.models.fields.related.ForeignKey', [], {'blank': 'True', 'related_name': "'files'", 'null': 'True', 'to': "orm['foia.FOIARequest']"}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
        ...
        'foia.foiamultirequest': {
            ...
            'agencies': ('django.db.models.fields.related.ManyToManyField', [], {'blank': 'True', 'related_name': "'agencies'", 'null': 'True', 'symmetrical': 'False', 'to': "orm['agency.Agency']"}),
