...
class DropboxDownloader(object):
    ...
    name = 'DropBox'
    p_link = re.compile(
        r'https://www.dropbox.com/[a-zA-Z0-9$_.+!*\'(),;/?:@=&-]+'
    )
...
def download_links(communication):
    ...
    logger.info(
        'Trying to download links for communication %s', communication.pk
    )

    for downloader in downloaders:
        logger.info(
            '[DL:%s] Looking for %s links', communication.pk, downloader.name
        )
