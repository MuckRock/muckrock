...
class Importer(object):
    ...
    def _match_jurisdiction(self, datum):
        ...
        if ',' in jurisdiction_name:
            locality, state = [
                j.strip() for j in jurisdiction_name.split(',', 1)
            ]
    ...
    def _match_one(self, datum):
        ...
        agencies = Agency.objects.get_approved().filter(
    ...
    def _import_fax(self, agency, datum):
        ...
        if fax:
            ...
            if datum['agency_status'] == 'created':
                ...
                if any(
                    p == fax_number for p in agency.phones.filter(type="fax")
                ):
    ...
    def _import_one(self, datum):
        agency = datum.get("match_agency")
        if agency is None:
            agency = self._create_agency(datum)
    ...
    def import_(self, dry=False):
        with transaction.atomic():
            for datum in self.match():
                yield self._import_one(datum)
