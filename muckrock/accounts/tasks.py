...
def store_statistics():
    ...
    stats = Statistics.objects.create(
        ...
        total_requests_success=FOIARequest.objects.filter(status='done'
                                                          ).count(),
        total_requests_denied=FOIARequest.objects.filter(status='rejected'
                                                         ).count(),
        total_requests_draft=FOIARequest.objects.filter(status='started'
                                                        ).count(),
        total_requests_submitted=FOIARequest.objects.filter(status='submitted'
                                                            ).count(),
        total_requests_awaiting_ack=FOIARequest.objects.filter(status='ack'
                                                               ).count(),
        total_requests_awaiting_response=FOIARequest.objects.filter(
            status='processed'
        ).count(),
        total_requests_awaiting_appeal=FOIARequest.objects.filter(
            status='appealing'
        ).count(),
        total_requests_fix_required=FOIARequest.objects.filter(status='fix'
                                                               ).count(),
        total_requests_payment_required=FOIARequest.objects.filter(
            status='payment'
        ).count(),
        total_requests_no_docs=FOIARequest.objects.filter(status='no_docs'
                                                          ).count(),
        total_requests_partial=FOIARequest.objects.filter(status='partial'
                                                          ).count(),
        total_requests_abandoned=FOIARequest.objects.filter(status='abandoned'
                                                            ).count(),
        total_requests_lawsuit=FOIARequest.objects.filter(status='lawsuit'
                                                          ).count(),
        ...
        machine_requests_success=FoiaMachineRequest.objects.filter(
            status='done'
        ).count(),
        machine_requests_denied=FoiaMachineRequest.objects.filter(
            status='rejected'
        ).count(),
        machine_requests_draft=FoiaMachineRequest.objects.filter(
            status='started'
        ).count(),
        machine_requests_submitted=FoiaMachineRequest.objects.filter(
            status='submitted'
        ).count(),
        machine_requests_awaiting_ack=FoiaMachineRequest.objects.filter(
            status='ack'
        ).count(),
        machine_requests_awaiting_response=FoiaMachineRequest.objects.filter(
            status='processed'
        ).count(),
        machine_requests_awaiting_appeal=FoiaMachineRequest.objects.filter(
            status='appealing'
        ).count(),
        machine_requests_fix_required=FoiaMachineRequest.objects.filter(
            status='fix'
        ).count(),
        machine_requests_payment_required=FoiaMachineRequest.objects.filter(
            status='payment'
        ).count(),
        machine_requests_no_docs=FoiaMachineRequest.objects.filter(
            status='no_docs'
        ).count(),
        machine_requests_partial=FoiaMachineRequest.objects.filter(
            status='partial'
        ).count(),
        machine_requests_abandoned=FoiaMachineRequest.objects.filter(
            status='abandoned'
        ).count(),
        machine_requests_lawsuit=FoiaMachineRequest.objects.filter(
            status='lawsuit'
        ).count(),
        total_pages=FOIAFile.objects.aggregate(Sum('pages'))['pages__sum'],
        total_users=User.objects.count(),
        total_users_excluding_agencies=User.objects.exclude(
            profile__acct_type='agency'
        ).count(),
        total_users_filed=User.objects.annotate(num_foia=Count('foiarequest')
        ...
        total_unresolved_tasks=Task.objects.filter(resolved=False
                                                   ).get_undeferred().count(),
        total_deferred_tasks=Task.objects.get_deferred().count(),
        total_generic_tasks=GenericTask.objects.count(),
        total_unresolved_generic_tasks=GenericTask.objects.filter(
            resolved=False
        ).get_undeferred().count(),
        total_deferred_generic_tasks=GenericTask.objects.get_deferred().count(),
        total_orphan_tasks=OrphanTask.objects.count(),
        total_unresolved_orphan_tasks=OrphanTask.objects.filter(
            resolved=False
        ).get_undeferred().count(),
        total_deferred_orphan_tasks=OrphanTask.objects.get_deferred().count(),
        total_snailmail_tasks=SnailMailTask.objects.count(),
        total_unresolved_snailmail_tasks=SnailMailTask.objects.filter(
            resolved=False
        ).get_undeferred().count(),
        total_deferred_snailmail_tasks=SnailMailTask.objects.get_deferred()
        .count(),
        total_rejected_tasks=RejectedEmailTask.objects.count(),
        total_unresolved_rejected_tasks=RejectedEmailTask.objects.filter(
            resolved=False
        ).get_undeferred().count(),
        total_deferred_rejected_tasks=RejectedEmailTask.objects.get_deferred()
        .count(),
        total_staleagency_tasks=StaleAgencyTask.objects.count(),
        total_unresolved_staleagency_tasks=StaleAgencyTask.objects.filter(
            resolved=False
        ).get_undeferred().count(),
        total_deferred_staleagency_tasks=StaleAgencyTask.objects.get_deferred()
        .count(),
        total_flagged_tasks=FlaggedTask.objects.count(),
        total_unresolved_flagged_tasks=FlaggedTask.objects.filter(
            resolved=False
        ).get_undeferred().count(),
        total_deferred_flagged_tasks=FlaggedTask.objects.get_deferred().count(),
        total_newagency_tasks=NewAgencyTask.objects.count(),
        total_unresolved_newagency_tasks=NewAgencyTask.objects.filter(
            resolved=False
        ).get_undeferred().count(),
        total_deferred_newagency_tasks=NewAgencyTask.objects.get_deferred()
        .count(),
        total_response_tasks=ResponseTask.objects.count(),
        total_unresolved_response_tasks=ResponseTask.objects.filter(
            resolved=False
        ).get_undeferred().count(),
        total_deferred_response_tasks=ResponseTask.objects.get_deferred()
        .count(),
        total_faxfail_tasks=FailedFaxTask.objects.count(),
        total_unresolved_faxfail_tasks=FailedFaxTask.objects.filter(
            resolved=False
        ).get_undeferred().count(),
        total_deferred_faxfail_tasks=FailedFaxTask.objects.get_deferred()
        .count(),
        total_crowdfundpayment_tasks=CrowdfundTask.objects.count(),
        total_unresolved_crowdfundpayment_tasks=CrowdfundTask.objects.filter(
            resolved=False
        ).get_undeferred().count(),
        total_deferred_crowdfundpayment_tasks=CrowdfundTask.objects.
        get_deferred().count(),
        total_reviewagency_tasks=ReviewAgencyTask.objects.count(),
        total_unresolved_reviewagency_tasks=ReviewAgencyTask.objects.filter(
            resolved=False
        ).get_undeferred().count(),
        total_deferred_reviewagency_tasks=ReviewAgencyTask.objects.get_deferred(
        ).count(),
        total_portal_tasks=PortalTask.objects.count(),
        total_unresolved_portal_tasks=PortalTask.objects.filter(
            resolved=False
        ).get_undeferred().count(),
        ...
        unresolved_snailmail_appeals=SnailMailTask.objects.filter(
            resolved=False, category='a'
        ).get_undeferred().count(),
        ...
        closed_crowdfunds_0=Crowdfund.objects.annotate(
            percent=F('payment_received') / F('payment_required')
        ).filter(closed=True, percent=0).count(),
        closed_crowdfunds_0_25=Crowdfund.objects.annotate(
            percent=F('payment_received') / F('payment_required')
        ).filter(closed=True, percent__gt=0, percent__lte=0.25).count(),
        closed_crowdfunds_25_50=Crowdfund.objects.annotate(
            percent=F('payment_received') / F('payment_required')
        ).filter(closed=True, percent__gt=0.25, percent__lte=0.50).count(),
        closed_crowdfunds_50_75=Crowdfund.objects.annotate(
            percent=F('payment_received') / F('payment_required')
        ).filter(closed=True, percent__gt=0.50, percent__lte=0.75).count(),
        closed_crowdfunds_75_100=Crowdfund.objects.annotate(
            percent=F('payment_received') / F('payment_required')
        ).filter(closed=True, percent__gt=0.75, percent__lte=1.00).count(),
        closed_crowdfunds_100_125=Crowdfund.objects.annotate(
            percent=F('payment_received') / F('payment_required')
        ).filter(closed=True, percent__gt=1.00, percent__lte=1.25).count(),
        closed_crowdfunds_125_150=Crowdfund.objects.annotate(
            percent=F('payment_received') / F('payment_required')
        ).filter(closed=True, percent__gt=1.25, percent__lte=1.50).count(),
        closed_crowdfunds_150_175=Crowdfund.objects.annotate(
            percent=F('payment_received') / F('payment_required')
        ).filter(closed=True, percent__gt=1.50, percent__lte=1.75).count(),
        closed_crowdfunds_175_200=Crowdfund.objects.annotate(
            percent=F('payment_received') / F('payment_required')
        ).filter(closed=True, percent__gt=1.75, percent__lte=2.00).count(),
        closed_crowdfunds_200=Crowdfund.objects.annotate(
            percent=F('payment_received') / F('payment_required')
        ).filter(closed=True, percent__gt=2.00).count(),
        total_crowdfund_payments=CrowdfundPayment.objects.count(),
        total_crowdfund_payments_loggedin=CrowdfundPayment.objects.exclude(
            user=None
        ).count(),
        total_crowdfund_payments_loggedout=CrowdfundPayment.objects.filter(
            user=None
        ).count(),
        public_projects=Project.objects.filter(private=False,
                                               approved=True).count(),
        private_projects=Project.objects.filter(private=True,
                                                approved=True).count(),
        ...
        project_users_pro=User.objects.filter(profile__acct_type='pro'
                                              ).exclude(projects=None).count(),
        project_users_basic=User.objects.filter(profile__acct_type='basic'
                                                ).exclude(projects=None
                                                          ).count(),
        project_users_beta=User.objects.filter(profile__acct_type='beta'
                                               ).exclude(projects=None).count(),
        project_users_proxy=User.objects.filter(profile__acct_type='proxy'
                                                ).exclude(projects=None
                                                          ).count(),
        project_users_admin=User.objects.filter(profile__acct_type='admin'
                                                ).exclude(projects=None
                                                          ).count(),
        ...
        total_draft_crowdsources=Crowdsource.objects.filter(status='draft'
                                                            ).count(),
        total_open_crowdsources=Crowdsource.objects.filter(status='open'
                                                           ).count(),
        total_close_crowdsources=Crowdsource.objects.filter(status='close'
                                                            ).count(),
        num_crowdsource_responded_users=CrowdsourceResponse.objects.aggregate(
