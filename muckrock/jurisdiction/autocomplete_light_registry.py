...
autocomplete_light.register(
    ...
    attrs={"placeholder": "State name?", "data-autocomplete-minimum-characters": 1},
)
...
class JurisdictionStateInclusiveAutocomplete(
    ...
    def choices_for_values(self):
        ...
        inc_local_values = [
            x.split("-")[0] for x in values if x != "" and x.split("-")[1] == "True"
        ]
        dont_inc_local_values = [
            x.split("-")[0] for x in values if x != "" and x.split("-")[1] == "False"
        ]
        inc_local_choices = self.choices.filter(
            pk__in=[x for x in inc_local_values]
        ).annotate(include_local=Value(True, output_field=BooleanField()))
        dont_inc_local_choices = self.choices.filter(
            pk__in=[x for x in dont_inc_local_values]
...
autocomplete_light.register(
    ...
    attrs={"placeholder": "Jurisdiction?", "data-autocomplete-minimum-characters": 2},
)
