...
class TestGeminiFileSearchService(TestCase):
    ...
    def setUp(self):
        ...
        from muckrock.jurisdiction.signals import upload_resource_to_gemini
        post_save.disconnect(upload_resource_to_gemini, sender=JurisdictionResource)

        self.state = factories.StateJurisdictionFactory(
            name="Colorado",
            abbrev="CO"
        )
    ...
    @patch('muckrock.jurisdiction.services.gemini_service.genai.Client')
    ...
    def test_upload_resource(self, mock_client):
        ...
        mock_operation.response.name = "files/test_file_123"
