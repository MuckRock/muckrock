...
def notify_flagged(sender, instance, created, **kwargs):
    ...
    def create_flagged_task_payload(flagged_task):
        ...
        if flagged_task.user is not None:
            flagged_by = slack_attachment(
                'Flagged by', format_user(flagged_task.user)
            )
            fields.append(flagged_by)
        flagged_object = {
            'title':
                '%s' % flagged_task.flagged_object().__class__.__name__,
            'value':
                '<%(url)s|%(name)s>' % {
                    'url':
                        base_url +
        ...
        return slack_message(
            ...
            'New <%s|flagged task>' % task_url, attachments
        )
...
def notify_project(sender, instance, created, **kwargs):
    ...
    def create_project_task_payload(task):
        ...
        names = [
            format_user(contributor)
            for contributor in task.project.contributors.all()
        ]
        ...
        return slack_message(
            ':file_cabinet:', '#tasks', 'New <%s|pending project>' % task_url,
            attachments
        )
...
post_save.connect(
    flagged,
    sender=FlaggedTask,
    dispatch_uid='muckrock.task.signals.flagged',
)
