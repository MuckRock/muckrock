...
class TestProjectEditView(TestCase):
    ...
    def test_anonymous(self):
        ...
        assert response.status_code == 302, "The user should be redirected."
        assert (
            response.url == redirect_url
        ), "The user should be redirected to the login page."
    ...
    def test_edit_description(self):
        ...
        assert form.is_valid(), "The form should validate. %s" % form.errors
        http_post_response(self.url, self.view, data, self.contributor, **self.kwargs)
        self.project.refresh_from_db()
        assert self.project.description == desc, "The description should be updated."
    ...
    def test_add_contributors(self, mock_notify):
        ...
        http_post_response(self.url, self.view, data, self.contributor, **self.kwargs)
        self.project.refresh_from_db()
        assert self.project.has_contributor(new_contributor)
        assert self.project.has_contributor(self.contributor)
...
class TestProjectCrowdfundView(TestCase):
    ...
    def test_post(self):
        ...
        assert crowdfund.name == name
        assert crowdfund.description == description
        ...
        assert crowdfund.payment_capped == payment_capped
        assert crowdfund.date_due == date_due
        assert response.status_code == 302
