...
# MuckRock
from muckrock.factories import AgencyFactory, FOIARequestFactory, ProjectFactory
...
class UnitTestMap(TestCase):
    ...
    def test_fields(self):
        ...
        Maps should contain:
            ...
            * a project
        """
        ok_(self.map.title, 'Maps should have a title.')
        eq_(
            self.map.slug, slugify(self.map.title),
        ...
        eq_(
            self.map.center, expected_center,
        ...
        eq_(
            self.map.markers.count(), 0,
            'Maps should have a collection of markers that is empty by default.'
        )
        eq_(self.map.project, self.project, 'Maps should have a project.')
...
class UnitTestMarker(TestCase):
    ...
    def setUp(self):
        ...
        self.map = Map.objects.create(
            title='Test map', project=ProjectFactory()
        )
    ...
    def test_fields(self):
        ...
        """
        eq_(self.marker.map, self.map, 'Markers should be placed on a map.')
        eq_(self.marker.foia, self.foia, 'Markers should reference a request.')
        eq_(
            self.marker.point, '',
    ...
    def test_agency_location_default(self):
        ...
        filled_marker = Marker.objects.create(
            map=self.map, foia=self.foia, point=location2
        )
        eq_(
            empty_marker.point, location1,
