...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Deleting field 'FOIARequest.request'
        db.delete_column('foia_foiarequest', 'request')

        # Deleting field 'FOIARequest.response'
        db.delete_column('foia_foiarequest', 'response')
    
    
    def backwards(self, orm):
        
        # Adding field 'FOIARequest.request'
        db.add_column('foia_foiarequest', 'request', self.gf('django.db.models.fields.TextField')(default=''), keep_default=False)

        # Adding field 'FOIARequest.response'
        db.add_column('foia_foiarequest', 'response', self.gf('django.db.models.fields.TextField')(default='', blank=True), keep_default=False)

        for foia in orm.FOIARequest.objects.all():
            comms = foia.communications.all()
            if len(comms) > 0:
                foia.request = comms[0].communication
            if len(comms) > 1:
                foia.response = comms[1].communication
